 ...
 * @Last Modified time: 2022-11-18 11:31:44
 */
...
extern "C" homekit_characteristic_t cha_sprinkler_active;
extern "C" homekit_characteristic_t cha_sprinkler_program;
extern "C" homekit_characteristic_t cha_in_use;
...
void set_lock(const homekit_value_t value) {
  ...
  cha_sprinkler_active.value.int_value = state;
  ...
  homekit_characteristic_notify(&cha_sprinkler_active, cha_sprinkler_active.value);
}

void my_homekit_setup() {

  cha_sprinkler_active.setter = set_lock;
...
void onPressed() {
  ...
  //   if (active == 0) {
  //     cha_sprinkler_active.value.int_value = 1;
  //     active = 1;
  //   } else if (active == 1) {
  //     cha_sprinkler_active.value.int_value = 0;
  //     active = 0;
  //   }
  ...
  if (program == 0) {
    cha_sprinkler_program.value.int_value = 1;
    program = 1;
  } else if (program == 1) {
    cha_sprinkler_program.value.int_value = 2;
    program = 2;
  } else if (program = 2) {
    cha_sprinkler_program.value.int_value = 0;
    program = 0;
  }

  //report the lock-mechanism current-sate to HomeKit
  homekit_characteristic_notify(&cha_sprinkler_program, cha_sprinkler_program.value);
}
// 长按事件函数
